...
# @@File             :  entrypoint.sh
# @@Description      :  Entrypoint file for stikked
...
#SERVICE_USER="${SERVICE_USER:-stikked}"   # execute command as another user
#SERVICE_GROUP="${SERVICE_GROUP:-stikked}" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Secondary ports
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  ...
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    [ "$ENTRYPOINT_MESSAGE" = "yes" ] && echo "Executing entrypoint script for stikked"
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - -
