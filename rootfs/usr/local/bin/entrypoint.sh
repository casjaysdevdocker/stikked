...
# @@Created          :  Thursday, Aug 15, 2024 16:52 EDT
# @@File             :  entrypoint.sh
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Server directories
WWW_ROOT_DIR="${WWW_ROOT_DIR:-/usr/local/share/httpd/default}" # set default web dir
DATABASE_DIR="${DATABASE_DIR:-/data/db/${DBTYPE:-sqlite}}"     # set database dir
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Healthcheck variables
HEALTH_ENABLED="yes" # enable healthcheck [yes/no]
SERVICES_LIST="tini,php-fpm,nginx"
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  ...
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    [ "$ENTRYPOINT_MESSAGE" = "yes" ] && echo "Executing entrypoint script for pastebin"
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
